...
IDP.phewas=readRDS('data/IDP_phewas_discovery.rds')
      count=data.frame(pheno=colnames(IDP.phewas),N=colSums(!is.na(IDP.phewas)))
      IDP.phewas=IDP.phewas[,count$N>=2000]

IDP.long_phewas=readRDS('data/dat_long_phewas_discovery.rds')
...
# covs
      ...
      ls.models$covs[ls.models$covs=='']=paste0(colnames(targetdata)[c(8:26)],collapse = '+')
...
# Analysis ----------------------------------------------------------------
result.discovery=reg_phewasStyle(ls.models,dat_short=targetdata,dat_long=dat_long)

save(result.discovery,file='result/i.Main_result/result_discovery.RData')
save(ls.models,file='script/ANALY.main/ls.models.RData')
