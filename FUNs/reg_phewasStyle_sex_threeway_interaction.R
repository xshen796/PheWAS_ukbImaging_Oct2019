reg_phewasStyle_interaction <- function (ls.models,dat_short,dat_long,interact_var){
      
      run_model <- function(ls.mod,dat_short,dat_long,interact_var){
            # define vars
            ...
            intvar_inData = dat_short[,interact_var]
            if (length(table(intvar_inData))!=2&(!is.factor(intvar_inData))){
                  interact_var=paste0('scale(',interact_var,')')
            }
            ...
            if (mod.type=='lme'){
                  ...
                  mod=paste0(dep,'~',covs,'+',factor,'*',interact_var)
      ...
      tmp.result = pbapply(X = ls.models,MARGIN = 1,FUN = run_model,dat_short=targetdata,dat_long=dat_long,interact_var=interact_var)
