...
# E factors
ls.Env=c('sociodemo.Household_income','sociodemo.TDI.Tertiles','mh.Trauma.Childhood','mh.Trauma.Adult','mh.SLE_current')
...
create_covs <- function(ls.keyvars,targetdata,env_var) {
      ...
      ls.covs = switch(
            cov.type,
            colnames(targetdata)[4:26],
            c(colnames(targetdata)[4:26]),
            colnames(targetdata)[4:26],
            c(colnames(targetdata)[c(4:26)],'ICV'),
            c(colnames(targetdata)[c(4:26)],'ICV'),
            c(colnames(targetdata)[c(4:26)],'ICV'),
            c(colnames(targetdata)[4:26],'r.motion'),
            colnames(targetdata)[c(4,8:26)]
            )
