...
source('FUNs/reg_phewasStyle_sex_threeway_interaction.R')
...
# E factors
ls.Env=c('sociodemo.TDI.Tertiles','mh.Trauma.Childhood')
ls.sig.dep=c('mh.ConditionSum_based_on_help_seeking','mh.Depression.Affective')
...
for (i in ls.Env){
      ...
      ls.models=ls.models[ls.models$dependent %in% ls.sig.dep,]
      ...
      save(list=obj.name,file=paste0('result/iv.PRS_by_E/PRS_X_sex_X',i,'.RData'))
}